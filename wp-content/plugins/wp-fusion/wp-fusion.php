...
define( 'WP_FUSION_VERSION', '3.45.10' );
$wpf_options = get_option( 'wpf_options' );
$wpf_options['license_key'] = '**************';
$wpf_options['license_status'] = 'valid';
$wpf_options['disabled'] = 'true';
...
final class WP_Fusion {
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'wp-fusion' ), esc_html( WP_FUSION_VERSION ) );
	}
	...
	public function load_textdomain() {
		...
		// Default languages directory for WP Fusion.
		$lang_dir = WPF_DIR_PATH . 'languages/';

		/**
		 * Filters the languages directory path to use for WP Fusion Pro.
		 *
		 * @param string $lang_dir The languages directory path.
		 */
		$lang_dir = apply_filters( 'wp_fusion_languages_directory', $lang_dir );

		$get_locale = get_user_locale();

		// Convert national variants to base locale
		$language   = substr( $get_locale, 0, 2 );
		$get_locale = $language . '_' . strtoupper( $language );

		/**
		 * Language Locale for WP Fusion.
		 *
		 * @var $get_locale The locale to use.
		 */
		$locale = apply_filters( 'plugin_locale', $get_locale, 'wp-fusion' );
		$mofile = sprintf( '%1$s-%2$s.mo', 'wp-fusion', $locale );
		...
		if ( file_exists( $mofile_global ) ) {
			// Look in global /wp-content/languages/wp-fusion/ folder.
			load_textdomain( 'wp-fusion', $mofile_global );
		} elseif ( file_exists( $mofile_local ) ) {
			// Look in local /wp-content/plugins/wp-fusion/languages/ folder.
			load_textdomain( 'wp-fusion', $mofile_local );
		} else {
			// Load the default language files.
			load_plugin_textdomain( 'wp-fusion', false, $lang_dir );
		}
	}
	...
	public function updater() {
		...
		// setup the updater
		new WPF_Plugin_Updater(
			WPF_STORE_URL,
			__FILE__,
			array(
				'version'   => WP_FUSION_VERSION,               // current version number.
				'license'   => wpf_get_option( 'license_key' ), // license key.
				'item_name' => 'WP Fusion',                     // name of this plugin.
				'author'    => 'Very Good Plugins',             // author of this plugin.
			)
		);

		if ( 'valid' !== wpf_get_option( 'license_status' ) ) {
			add_action( 'after_plugin_row_' . plugin_basename( __FILE__ ), array( $this, 'wpf_update_message' ), 10, 3 );
		}
	}
	...
	public function wpf_update_message( $plugin_file, $plugin_data, $status ) {
		...
		$update_message = sprintf(
			/* translators: 1: Activation link, 2: Purchase link */
			__( 'Your WP Fusion License key is currently inactive or expired. <a href="%1$s">Activate your license key</a> or <a href="%2$s" target="_blank">purchase a license</a> to enable automatic updates and support.', 'wp-fusion' ),
			esc_url( get_admin_url() . 'options-general.php?page=wpf-settings#setup' ),
			esc_url( 'https://wpfusion.com/pricing/' )
		);
		...
		echo '<p>' . wp_kses_post( $update_message ) . '</p>';
		echo '</div>';
		echo '</td>';
		echo '</tr>';
	}
