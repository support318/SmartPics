...
class WPF_Lite_Helper {
	...
	public function add_meta_field_group( $field_groups ) {
		...
		if ( class_exists( 'SFWD_LMS' ) ) {
			$field_groups['learndash_progress'] = array(
				'title'    => __( 'LearnDash - Progress', 'wp-fusion' ),
				'disabled' => true,
			);
		}
	...
	public function woocommerce_write_panels() {
		...
		echo '<p class="form-field"><label for="wpf-apply-tags-woo">' . esc_html__( 'Apply tags when purchased', 'wp-fusion' ) . '</label>';

		wpf_render_tag_multiselect( array( 'field_id' => 'apply_tags' ) );

		echo '<br /><span style="margin-left: 0px;" class="description show_if_variable">' . esc_html__( 'Tags for product variations can be configured within the variations tab.', 'wp-fusion' ) . '</span>';

		echo '</p>';

		echo '<p class="form-field"><label for="wpf-apply-tags-woo">' . esc_html__( 'Apply tags when refunded', 'wp-fusion' );
		echo ' <span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . esc_attr__( 'The tags specified above for \'Apply tags when purchased\' will automatically be removed if an order is refunded.', 'wp-fusion' ) . '"></span>';
		echo '</label>';
		...
		echo '<p class="form-field"><label for="wpf-apply-tags-woo">' . esc_html__( 'Apply tags when transaction failed', 'wp-fusion' );
		...
		if ( class_exists( 'WC_Subscriptions' ) ) {
			...
			echo '<span class="description">' . esc_html__( 'Apply these tags when free trial ends', 'wp-fusion' ) . '.</span>';
			echo '</p>';
		...
		echo '<h3>' . sprintf( esc_html__( 'Segment customers using tags in %s.', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ) . '</h3>';

		echo '<p>' . sprintf( esc_html__( 'With the full version of WP Fusion you can apply tags in %s based on product purchases, variations, coupon usage, order status changes, and more.', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ) . '</p>';

		if ( class_exists( 'WC_Subscriptions' ) ) {

			echo '<p>' . sprintf( esc_html__( 'With WooCommerce Subscriptions active you can also %1$sapply tags based on subscription status changes and payment failures%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/ecommerce/woocommerce-subscriptions/?utm_source=free-plugin&utm_medium=woocommerce&utm_campaign=free-plugin" target="_blank">', '</a>' ) . '</p>';

		}

		echo '<p>' . sprintf( esc_html__( '%1$sUpgrade WP Fusion today%2$s and automate your WooCommerce marketing with %3$s.', 'wp-fusion' ), '<strong>', '</strong>', esc_html( wp_fusion()->crm->name ) ) . '</p>';
	...
	public function contact_fields_upgrade_message( $id, $field ) {
		...
		$output .= sprintf( '<h2>%s</h2>', sprintf( __( 'Sync more data with %s.', 'wp-fusion' ), $crm_name ) );

		$output .= '<p>' . __( 'You\'re currently using <strong>WP Fusion Lite</strong>, which syncs your WordPress "core" fields with your CRM.', 'wp-fusion' ) . '</p>';

		$output .= '<p>' . __( 'Did you know that the full version of WP Fusion supports syncing data from ', 'wp-fusion' );
		...
		// translators: CRM name.
		$output .= sprintf(
			__( 'and %1$s bidirectionally with %2$s.</p>', 'wp-fusion' ),
			'<a href="https://wpfusion.com/documentation/?utm_campaign=free-plugin&utm_source=free-plugin&utm_medium=contact-fields#integrations" target="_blank">' . __( 'over 100 other plugins', 'wp-fusion' ) . '</a>',
			$crm_name
		);
		...
		$output .= sprintf(
			...
			__( 'Learn More About Syncing Custom Fields', 'wp-fusion' )
		);
		$output .= ' <span class="orange">' . __( 'or', 'wp-fusion' ) . '</span> ';
		$output .= sprintf(
	...
	public function total_syncs_notice( $notices ) {
		...
		if ( wpf_get_option( 'total_users_synced' ) > 10 ) {
			...
			$notices['total-synced'] = sprintf( __( '<strong>Congratulations!</strong> You\'ve successfully synced %1$d new user registrations to %2$s with WP Fusion! 🎉 Have a second? %3$sLeave us a review &rarr;%4$s ⭐⭐⭐⭐⭐', 'wp-fusion' ), intval( wpf_get_option( 'total_users_synced' ) ), esc_html( wp_fusion()->crm->name ), '<a href="https://wordpress.org/plugins/wp-fusion-lite/#reviews" target="_blank">', '</a>' );
