	...
	public function show_staging_notice() {
		...
		if ( $this->is_duplicate_site() && current_user_can( 'manage_options' ) && ! defined( 'WPF_STAGING_MODE' ) ) {
			...
			printf(
				...
				esc_html__( 'It looks like this site has moved or is a duplicate site. %1$sWP Fusion%2$s has enabled %7$sstaging mode%8$s to prevent unwanted data from being synced with %3$s. %1$sWP Fusion%2$s considers %4$s%6$s%5$s to be the site\'s URL.', 'wp-fusion' ),
				'<strong>',
				'</strong>',
			...
			if ( ! doing_action( 'wpf_settings_notices' ) ) {
				...
				esc_html_e( 'Quit nagging me (but keep staging mode enabled)', 'wp-fusion' );
			...
			esc_html_e( 'Disable staging mode and make this the production site', 'wp-fusion' );
		...
		if ( ( wpf_get_option( 'staging_mode' ) && ! $this->is_duplicate_site() ) || ( defined( 'WPF_STAGING_MODE' ) && true === WPF_STAGING_MODE ) ) {
			...
			if ( defined( 'WPF_STAGING_MODE' ) && true === WPF_STAGING_MODE ) {
				...
				printf(
					...
					esc_html__( '%1$sHeads up:%2$s WP Fusion is currently in %3$sstaging mode%4$s due to %5$sWPF_STAGING_MODE%6$s being defined in wp-config.php. No data will be sent to or loaded from %7$s.', 'wp-fusion' ),
					'<strong>',
					'</strong>',
					'<a href="https://wpfusion.com/documentation/faq/staging-sites/" target="_blank">',
					'</a>',
				...
				printf(
					// translators: 1$-2$: opening and closing <strong> tags. $3-$4 opening and closing link to documentation on staging sites. $5 the CRM name.
					esc_html__( '%1$sHeads up:%2$s WP Fusion is currently in %3$sstaging mode%4$s. No data will be sent to or loaded from %5$s.', 'wp-fusion' ),
					'<strong>',
					'</strong>',
					'<a href="https://wpfusion.com/documentation/faq/staging-sites/" target="_blank">',
					'</a>',
