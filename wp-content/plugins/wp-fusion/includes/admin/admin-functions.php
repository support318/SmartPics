...
function wpf_render_tag_multiselect( $args = array() ) {
	...
	if ( 1 === $args['limit'] ) {
		$args['placeholder'] = __( 'Select a tag', 'wp-fusion' );
	}
	...
	if ( ! empty( $unknown_tags ) ) {
		echo '<span class="wpf-unknown-tags">' . esc_html__( 'Unknown tag(s): ', 'wp-fusion' ) . esc_html( implode( ', ', $unknown_tags ) ) . '</span>';
	}
...
function wpf_render_crm_field_select( $setting, $meta_name, $field_id = false, $field_sub_id = false ) {
	...
	if ( in_array( 'add_tags', wp_fusion()->crm->supports ) ) {
		...
		echo '<optgroup label="Tagging">';
			...
			echo '<option ' . selected( $setting, 'add_tag_' . $field_id ) . ' value="add_tag_' . esc_attr( $field_id ) . '">+ ' . esc_html__( 'Create tag(s) from value', 'wp-fusion' ) . '</option>';
...
function wpf_status_icon( $complete_data, $type = 'entry' ) {
	...
	if ( $complete_data['contact_id'] ) {
		...
		$tooltip   = sprintf(
			...
			__( 'This %1$s was synced to %2$s contact ID %3$s on %4$s.', 'wp-fusion' ),
			esc_html( $type ),
		...
		if ( isset( $complete_data['ec_complete'] ) ) {
			...
			if ( $complete_data['ec_complete'] ) {
				...
				if ( $complete_data['ec_invoice_id'] ) {
					$tooltip .= '<br /><br />' . sprintf( __( 'It was processed by Enhanced Ecommerce with invoice ID #%s.', 'wp-fusion' ), $complete_data['ec_invoice_id'] );
				} else {
		...
		$tooltip = sprintf(
			...
			__( 'This %1$s was not synced to %2$s.', 'wp-fusion' ),
			esc_html( $type ),
			esc_html( wp_fusion()->crm->name )
		);
	}
