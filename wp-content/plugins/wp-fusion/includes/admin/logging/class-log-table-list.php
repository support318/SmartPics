...
class WPF_Log_Table_List extends WP_List_Table {
	...
	public function level_dropdown() {
		...
		$levels = array(
			array(
				'value' => 'error',
				'label' => __( 'Error', 'wp-fusion' ),
			),
			array(
				'value' => 'warning',
				'label' => __( 'Warning', 'wp-fusion' ),
			),
			array(
				'value' => 'notice',
				'label' => __( 'Notice', 'wp-fusion' ),
			),
			array(
				'value' => 'info',
				'label' => __( 'Info', 'wp-fusion' ),
			),
			array(
				'value' => 'http',
				'label' => __( 'HTTP', 'wp-fusion' ),
			),
		);

		$selected_level = isset( $_REQUEST['level'] ) ? esc_attr( $_REQUEST['level'] ) : '';
		?>
			<label for="filter-by-level" class="screen-reader-text"><?php esc_html_e( 'Filter by level', 'wp-fusion' ); ?></label>
			<select name="level" id="filter-by-level">
				<option<?php selected( $selected_level, '' ); ?> value=""><?php esc_html_e( 'All levels', 'wp-fusion' ); ?></option>
				<?php
				foreach ( $levels as $l ) {
					printf(
	...
	public function column_timestamp( $log ) {
		...
		$output .= sprintf( __( '<a class="wpf-log-link" href="%s"><span class="dashicons dashicons-admin-links"></span></a>', 'wp-fusion' ), $link );
		...
		if ( isset( $levels[ $level_key ] ) ) {
			...
			if ( isset( $log['context'] ) ) {
				...
				if ( isset( $context['request_uri'] ) && isset( $context['request_args'] ) ) {
					...
					$return .= ' <a href="' . esc_url( $link ) . '" class="wpf-logs-retry-link dashicons dashicons-update wpf-tip wpf-tip-right" data-tip="' . esc_attr__( 'Retry API call', 'wp-fusion' ) . '"></a>';
				}
			}
	...
	public function column_user( $log ) {
		...
		if ( $edit_url ) {
			$ret .= ' (<a title="' . sprintf( esc_attr__( 'View in %s', 'wp-fusion' ), wp_fusion()->crm->name ) . ' &raquo;" href="' . esc_url( $edit_url ) . '" target="_blank">#' . esc_html( $contact_id ) . '<span class="dashicons dashicons-external"></span></a>)';
		}
			...
			if ( ! empty( $context['meta_array'] ) ) {
				...
				foreach ( $context['meta_array'] as $key => $value ) {
					...
					if ( is_array( $value ) && isset( $value['pseudo'] ) ) {
						...
						$text = __( 'This field is a pseudo field or read-only field, and will not be saved to the database.', 'wp-fusion' );
						...
						$text = sprintf(
							...
							__( 'This value was modified by the wpf_format_field_value filter before being sent to %1$s, using field format %2$s.', 'wp-fusion' ),
							wp_fusion()->crm->name,
							$value['type']
						);
	...
	public function search_text() {
		echo '<input type="text" placeholder="' . esc_attr__( 'Search Logs', 'wp-fusion' ) . '" name="s" value="' . ( isset( $_REQUEST['s'] ) ? esc_attr( $_REQUEST['s'] ) : '' ) . '" />';
		submit_button( esc_html__( 'Search Logs', 'wp-fusion' ), '', 'search-action', false );
	}
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			echo '<div class="alignleft actions">';
				...
				$this->user_dropdown();
				$this->date_select();
				submit_button( esc_html__( 'Filter', 'wp-fusion' ), '', 'filter-action', false );
			echo '</div>';
		}
	}
	...
	protected function source_dropdown() {
		...
		if ( ! empty( $sources_db ) ) {
			...
			?>
				<label for="filter-by-source" class="screen-reader-text"><?php esc_html_e( 'Filter by source', 'wp-fusion' ); ?></label>
				<select name="source" id="filter-by-source">
					<option<?php selected( $selected_source, '' ); ?> value=""><?php esc_html_e( 'All sources', 'wp-fusion' ); ?></option>
					<?php
	...
	protected function user_dropdown() {
		...
		?>
			<label for="filter-by-user" class="screen-reader-text"><?php esc_html_e( 'Filter by user', 'wp-fusion' ); ?></label>
			<select class="select4-users-log" name="user" id="filter-by-user">
				<option value=""><?php esc_html_e( 'All users', 'wp-fusion' ); ?></option>
				<?php
	...
	protected function date_select() {
		...
		?>
			<label for="start-date" class="screen-reader-text"><?php esc_html_e( 'Start date', 'wp-fusion' ); ?></label>

			<?php if ( ! empty( $_GET['startdate'] ) ) : ?>
				<input placeholder="<?php esc_attr_e( 'Start date', 'wp-fusion' ); ?>" name="startdate" id="start-date" type="date" value="<?php echo esc_attr( $_GET['startdate'] ); ?>" />
			<?php else : ?>
				<input placeholder="<?php esc_attr_e( 'Start date', 'wp-fusion' ); ?>" name="startdate" id="start-date" type="text" onfocus="(this.type='date')" />
			<?php endif; ?>

			<label for="end-date" class="screen-reader-text"><?php esc_html_e( 'End date', 'wp-fusion' ); ?></label>

			<?php if ( ! empty( $_GET['enddate'] ) ) : ?>
				<input placeholder="<?php esc_attr_e( 'End date', 'wp-fusion' ); ?>" name="enddate" id="end-date" type="date" value="<?php echo esc_attr( $_GET['enddate'] ); ?>" />
			<?php else : ?>
				<input placeholder="<?php esc_attr_e( 'End date', 'wp-fusion' ); ?>" name="enddate" id="end-date" type="text" onfocus="(this.type='date')" />
			<?php endif; ?>
