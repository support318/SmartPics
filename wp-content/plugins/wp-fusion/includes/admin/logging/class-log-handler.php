...
class WPF_Log_Handler {
	...
	public function register_logger_subpage() {
		...
		$menu_title = __( 'WP Fusion Logs', 'wp-fusion' );
		...
		if ( wpf_get_option( 'logging_badge', true ) && ( ! isset( $_GET['page'] ) || 'wpf-settings-logs' !== $_GET['page'] ) ) {
			...
			if ( $errors_count ) {
				...
				$menu_title .= ' <span title="' . esc_attr( __( 'New WP Fusion API Errors', 'wp-fusion' ) ) . '" class="awaiting-mod count-' . $errors_count . '">' . $errors_count . '</span>';
				...
				if ( ! empty( $menu_item ) ) {
					...
					$menu[ $menu_item_position ][0] .= ' <span title="' . esc_attr( __( 'New WP Fusion API Errors', 'wp-fusion' ) ) . '" class="awaiting-mod count-' . $errors_count . '">' . $errors_count . '</span>';
				}
			}
		}
		...
		$page = add_submenu_page(
			'tools.php',
			__( 'WP Fusion Activity Logs', 'wp-fusion' ),
			$menu_title,
			'manage_options',
			'wpf-settings-logs',
	...
	public function configure_sections( $page, $options ) {
		...
		if ( wpf_get_option( 'logging_badge', true ) ) {
			...
			if ( $errors_count ) {
				...
				// Add it to the WPF Logs menu item.
				$title .= ' <span title="' . esc_attr__( 'New WP Fusion API Errors', 'wp-fusion' ) . '" class="awaiting-mod count-' . $errors_count . '">' . $errors_count . '</span>';
	...
	public function retry_api_call() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to perform this action.', 'wp-fusion' ) );
		}

		// Verify nonce
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'wpf_retry_log' ) ) {
			wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-fusion' ) );
		}
		...
		if ( ! $log ) {
			wp_die( esc_html__( 'Log entry not found.', 'wp-fusion' ) );
		}

		$context = maybe_unserialize( $log->context );

		if ( empty( $context['request_uri'] ) || empty( $context['request_args'] ) ) {
			wp_die( esc_html__( 'No API request data found in log entry.', 'wp-fusion' ) );
		}
	...
	public function show_logs_section() {
		...
		<div class="wrap">
			...
			<form method="get" id="mainform">
				...
				<h1 class="wp-heading-inline"><?php esc_html_e( 'WP Fusion Activity Log', 'wp-fusion' ); ?></h1>
				...
				<input style="vertical-align: baseline;" type="submit" name="wpf-flush-logs" id="flush-logs" class="button delete" value="<?php esc_attr_e( 'Flush all logs', 'wp-fusion' ); ?>">
				...
				<span class="description" style="display: inline-block; padding: 5px 0;">
					<?php printf( esc_html__( 'For more information on the logs, %1$ssee our documentation%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/getting-started/activity-logs/" target="_blank">', '</a>' ); ?>
					<?php printf( esc_html__( 'To go back to the main settings page, %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings' ) ) . '">', '</a>' ); ?>
				</span>
				...
				<?php submit_button( esc_html__( 'Flush all logs', 'wp-fusion' ), 'delete', 'wpf-flush-logs' ); ?>
				...
				<?php if ( ! function_exists( 'fatal_error_notify' ) ) : ?>
					...
					<div id="fen-pro">
						...
						<div class="fen-pro-center">
							...
							<p><?php printf( esc_html__( '%1$sFatal Error Notify%2$s can send you an email, SMS, or Slack message when WP Fusion encounters an API error.', 'wp-fusion' ), '<a href="https://fatalerrornotify.com/?utm_campaign=wpfusion&utm_source=wp-fusion-logs" target="_blank">', '</a>' ); ?></p>

							<p><?php esc_html_e( 'You can also set up notifications for other plugins, or WordPress itself.', 'wp-fusion' ); ?></p>

							<p><?php printf( esc_html__( 'Take 50%% off your first year using coupon code %1$sWP-FUSION%2$s.', 'wp-fusion' ), '<code>', '</code>' ); ?></p>
	...
	public function handle( $level, $user, $message, $context = array() ) {
		...
		$message = wp_fusion()->settings->set_tag_labels( $message, false, 'wp-fusion' );
	...
	public function shutdown() {
		...
		} elseif ( false !== strpos( $error['file'], 'wp-fusion' ) || false !== strpos( $error['message'], 'wp-fusion' ) ) {
