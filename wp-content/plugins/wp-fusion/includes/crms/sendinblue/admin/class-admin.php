...
class WPF_SendinBlue_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['sendinblue_key'] = array(
			...
			'desc'        => __( 'You can find your API v3 key in the <a href="https://account.brevo.com/advanced/api/" target="_blank">API settings</a> of your Brevo account.', 'wp-fusion' ),
			'type'        => 'api_validate',
			'section'     => 'setup',
			'class'       => 'api_key',
	...
	public function register_settings( $settings, $options ) {
		...
		$site_tracking['site_tracking_header'] = array(
			'title'   => __( 'Brevo Site Tracking', 'wp-fusion' ),
		...
		$site_tracking['site_tracking'] = array(
			...
			'desc'    => __( 'Enable Brevo site tracking scripts.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'main',
		);

		$site_tracking['site_tracking_key'] = array(
			'title'   => __( 'Tracking Client Key', 'wp-fusion' ),
			'desc'    => __( 'Your tracking <code>client_key</code> can be found in the Tracking Code <a href="https://automation.brevo.com/parameters" target="_blank">in the Automation settings of your Brevo account</a>. For example: <code>l7u0448l6oipghl8v7k92</code>', 'wp-fusion' ),
			'type'    => 'text',
			'section' => 'main',
		);

		$site_tracking['double_optin_header'] = array(
			'title'   => __( 'Double Opt-in Settings', 'wp-fusion' ),
		...
		$site_tracking['double_optin_template'] = array(
			'title'       => __( 'Double Opt-in Template', 'wp-fusion' ),
			'desc'        => __( 'Select a template to use for double opt-in. For more information see the <a href="https://help.brevo.com/hc/en-us/articles/360019540880-Create-a-double-opt-in-DOI-confirmation-template-for-Brevo-form" target="_blank">Brevo documentation</a>.', 'wp-fusion' ),
			'type'        => 'select',
			'placeholder' => __( 'None', 'wp-fusion' ),
		...
		$site_tracking['double_optin_redirect_url'] = array(
			'title'   => __( 'Opt-in Redirect URL', 'wp-fusion' ),
			'desc'    => __( 'URL that user will be redirected to after clicking on the double opt-in URL. When editing your DOI template you can reference this URL by using <code>{{ params.DOIurl }}</code>.', 'wp-fusion' ),
			'type'    => 'text',
			'std'     => home_url(),
			'section' => 'main',
		);
		...
		$site_tracking['double_optin_lists'] = array(
			'title'   => __( 'Opt-in List(s)', 'wp-fusion' ),
			'desc'    => __( 'You must select at least one list to add subscribers to once they have confirmed their subscription.', 'wp-fusion' ),
			'type'    => 'assign_tags',
			'section' => 'main',
		);
	...
	public function validate_double_optin( $value, $field, $options_class ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( false === filter_var( $options_class->post_data['double_optin_redirect_url'], FILTER_VALIDATE_URL ) ) {
				return new WP_Error( 'invalid_redirect_url', __( 'You must enter a valid redirect URL.', 'wp-fusion' ) );
			} elseif ( empty( $options_class->post_data['double_optin_lists'] ) ) {
				return new WP_Error( 'no_optin_lists', __( 'You must select at least one list to add subscribers to once they have confirmed their subscription.', 'wp-fusion' ) );
			}
		}
