...
class WPF_FluentCRM_REST_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['fluentcrm_rest_url'] = array(
			...
			'section' => 'setup',
			'class'   => 'wp-rest-url',
			'desc'    => __( 'Enter the URL to your website where FluentCRM is installed (must be https://)', 'wp-fusion' ),
		);
		...
		$new_settings['fluentcrm_rest_url']['desc'] .= '<br /><br /><a id="fluentcrm_rest-auth-btn" class="' . $class . '" href="' . $href . '">' . __( 'Authorize with FluentCRM', 'wp-fusion' ) . '</a>';
		$new_settings['fluentcrm_rest_url']['desc'] .= '<span class="description">' . __( 'You can click the Authorize button to be taken to the FluentCRM site and generate an application password automatically.', 'wp-fusion' ) . '</span>';

		$new_settings['fluentcrm_tag_format'] = array(
			'title'   => __( 'Tag Format', 'wp-fusion' ),
			'type'    => 'select',
			'section' => 'setup',
			'desc'    => sprintf( __( 'Select how FluentCRM tags should be %1$sreferenced by WP Fusion%2$s. For new installs we recommend using IDs, slugs are provided as an option for backwards compatibility.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/installation-guides/how-to-connect-fluentcrm-rest-api-to-wordpress/#tag-format" target="_blank">', '</a>' ),
			'std'     => ! empty( $options['connection_configured'] ) ? 'slug' : 'id',
			'choices' => array(
				'id'   => 'IDs',
				'slug' => 'Slugs',
			),
			'tooltip' => __( 'After changing the tag format, you will need to Resync Tags for Every User from the Advanced settings tab to load the new tags.', 'wp-fusion' ),
		);

		$new_settings['fluentcrm_rest_username'] = array(
			'title'   => __( 'Application Username', 'wp-fusion' ),
			'type'    => 'text',
			'section' => 'setup',
		);

		$new_settings['fluentcrm_rest_password'] = array(
			'title'       => __( 'Application Password', 'wp-fusion' ),
			'type'        => 'api_validate',
			'section'     => 'setup',
			'class'       => 'api_key',
	...
	public function configure_settings( $settings ) {
		...
		$new_settings = array(
			'default_status' => array(
				...
				'tooltip' => __( 'If Pending is selected, a double opt-in email will be sent to confirm the subscriber\'s email address. This can be overridden on a per-form basis by syncing a value of "subscribed" to the Status field.', 'wp-fusion' ),
				'type'    => 'select',
				'std'     => 'subscribed',
				'section' => 'main',
				'choices' => array(
		...
		if ( isset( $settings['email_optin_tags'] ) ) {
			...
			$new_settings = array(
				'woo_optin_status' => array(
					'title'   => __( 'Optin Status', 'wp-fusion' ),
					'desc'    => __( 'Select an opt-in status for customers who check the optin box.', 'wp-fusion' ),
					'tooltip' => __( 'If Pending is selected, a double opt-in email will be sent to confirm the subscriber\'s email address.', 'wp-fusion' ),
					'type'    => 'select',
					'std'     => 'subscribed',
