...
class WPF_Infusionsoft_iSDK {
	...
	public function api_key_warning() {
		...
		if ( false === strpos( wpf_get_option( 'api_key' ), 'KeapAK-' ) ) {
			...
			echo wp_kses_post( sprintf( __( '<strong>Heads up:</strong> WP Fusion\'s Infusionsoft / Keap integration has been updated to use Service Account Keys. Please %1$sgenerate a new Service Account Key%2$s and update it on the %3$sSetup tab in the WP Fusion settings%2$s to avoid service interruption.', 'wp-fusion' ), '<a href="https://developer.infusionsoft.com/pat-and-sak/">', '</a>', '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings#setup' ) ) . '">' ) );
		...
		if ( get_transient( 'wpf_keap_backup_key' ) ) {
			echo '<div class="notice notice-warning wpf-notice"><p>';
			echo wp_kses_post( __( '<strong>Note:</strong> WP Fusion is currently using your backup Infusionsoft Service Account Key due to API throttling on the primary key. It will switch back to the primary key at 12am UTC.', 'wp-fusion' ) );
			echo '</p></div>';
		}
	}
		...
		if ( strpos( $url, 'https://api.infusionsoft.com/crm/rest/' ) !== false && 'WP Fusion; ' . home_url() == $args['user-agent'] ) {
			...
			if ( 200 === $response_code || 201 === $response_code || 204 === $response_code ) {
				...
				if ( $request_body && isset( $request_body['owner_id'] ) ) {
					...
					$response = new WP_Error( 'error', sprintf( __( 'Owner ID %s is not valid.', 'wp-fusion' ), $request_body['owner_id'] ) );
		...
		foreach ( $fields_mapping['Emails'] as $old_email => $new_email ) {
			if ( isset( $data[ $old_email ] ) ) {
				...
				$crm_data['email_addresses'][] = array(
					...
					'field'         => $new_email,
					'opt_in_reason' => __( 'Contact was opted in through the WP Fusion integration.', 'wp-fusion' ),
				);
