...
class WPF_Salesforce_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['salesforce_header'] = array(
			...
			'title'   => sprintf( __( '%s Configuration', 'wp-fusion' ), $this->name ),
		...
		if ( empty( $options['sf_access_token'] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings['salesforce_header']['desc'] .= '<th scope="row"><label>' . esc_html__( 'Authorize', 'wp-fusion' ) . '</label></th>';
			$new_settings['salesforce_header']['desc'] .= '<td><a id="dynamics-auth-btn" class="button button-primary" href="' . esc_url( $this->crm->get_oauth_url() ) . '">' . sprintf( esc_html__( 'Authorize with %s', 'wp-fusion' ), $this->name ) . '</a><br />';
			$new_settings['salesforce_header']['desc'] .= '<span class="description">' . sprintf( esc_html__( 'You\'ll be taken to %s to authorize WP Fusion and generate access keys for this site.', 'wp-fusion' ), $this->name ) . '</td>';
			$new_settings['salesforce_header']['desc'] .= '</tr></table>';

			if ( ! is_ssl() ) {
				$new_settings['salesforce_header']['desc'] .= '<p class="wpf-notice notice notice-error">' . sprintf( esc_html__( '<strong>Warning:</strong> Your site is not currently SSL secured (https://). You will not be able to connect to the %s API. Your Site Address must be set to https:// in Settings &raquo; General.', 'wp-fusion' ), $this->name ) . '</p>';
			}
			...
			$new_settings['sf_instance_url'] = array(
				'title'   => __( 'Instance URL', 'wp-fusion' ),
				'type'    => 'text',
				'section' => 'setup',
			);
			...
			if ( ! empty( $options['connection_configured'] ) && 'salesforce' === wpf_get_option( 'crm' ) ) {
				...
				$new_settings['sf_tag_type'] = array(
					'title'   => __( 'Segment Type', 'wp-fusion' ),
					'std'     => 'Topics',
					'type'    => 'radio',
					'section' => 'setup',
					'choices' => array(
					...
					'desc'    => __( 'For more information, see <a href="https://wpfusion.com/documentation/crm-specific-docs/salesforce-tags/" target="_blank">Tags with Salesforce</a>.', 'wp-fusion' ),
				);

				$new_settings['sf_tag_picklist'] = array(
					'title'    => __( 'Tags Picklist', 'wp-fusion' ),
					'disabled' => isset( $options['sf_tag_type'] ) && 'Picklist' === $options['sf_tag_type'] ? false : true,
					'type'     => 'crm_field',
					'section'  => 'setup',
					'desc'     => __( 'Select a picklist type field to be used for segmentation with WP Fusion. For more information, see <a href="https://wpfusion.com/documentation/crm-specific-docs/salesforce-tags/" target="_blank">Tags with Salesforce</a>.', 'wp-fusion' ),
				);

				$new_settings['sf_object_type'] = array(
					'title'   => __( 'Object Type', 'wp-fusion' ),
					'type'    => 'select',
					'section' => 'setup',
					'choices' => get_option( 'wpf_salesforce_objects', array() ),
					'std'     => $this->crm->object_type,
					'desc'    => __( 'Select an object type to use with WP Fusion.', 'wp-fusion' ),
				);
				...
				if ( ! empty( $record_types ) ) {
					$new_settings['sf_record_type'] = array(
						'title'       => __( 'Record Type', 'wp-fusion' ),
						// translators: %s is the object type.
						'desc'        => sprintf( __( 'Select a record type to be used when creating new %s.', 'wp-fusion' ), strtolower( $this->crm->object_type ) . 's' ),
						'section'     => 'setup',
						'type'        => 'select',
						'placeholder' => __( 'Select one', 'wp-fusion' ),
			...
			$new_settings['sf_refresh_token'] = array(
				...
				'desc'           => '<a href="' . esc_url( $this->crm->get_oauth_url() ) . '">' . sprintf( esc_html__( 'Re-authorize with %s', 'wp-fusion' ), $this->crm->name ) . '</a>. ',
			);

			$new_settings['sf_refresh_token']['desc'] .= __( 'To avoid having to repeatedly re-authorize, make sure the WP Fusion app is <a href="https://wpfusion.com/documentation/installation-guides/how-to-connect-salesforce-to-wordpress/#complete-installation" target="_blank">completely installed</a>.', 'wp-fusion' );
	...
	public function register_settings( $settings, $options ) {
		...
		$new_settings['salesforce_account'] = array(
			'title'       => __( 'Default Account', 'wp-fusion' ),
			'desc'        => __( 'You can optionally enter a default account ID here to be used for new contact records. You can see the account ID in the URL when editing any Account record in Salesforce.', 'wp-fusion' ),
			'type'        => 'text',
			'placeholder' => __( 'Account ID', 'wp-fusion' ),
			'section'     => 'main',
		);
