...
class WPF_Autonami_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['autonami_url'] = array(
			...
			'section' => 'setup',
			'class'   => 'wp-rest-url',
			'desc'    => __( 'Enter the URL to your website where FunnelKit Automations is installed (must be https://).', 'wp-fusion' ),
		);

		if ( class_exists( 'BWFAN_Core' ) ) {
			$new_settings['autonami_url']['desc'] .= '<br /><br /><strong>' . sprintf( __( 'If you are trying to connect to FunnelKit Automations on this site, enter %s for the URL.', 'wp-fusion' ), '<code>' . home_url() . '</code>' ) . '</strong>';
		}
		...
		$new_settings['autonami_url']['desc'] .= '<br /><br /><a id="autonami-auth-btn" class="' . esc_attr( $class ) . '" href="' . esc_url( $href ) . '">' . __( 'Authorize with FunnelKit Automations', 'wp-fusion' ) . '</a>';
		$new_settings['autonami_url']['desc'] .= '<span class="description">' . __( 'You can click the Authorize button to be taken to the FunnelKit Automations site and generate an application password automatically.', 'wp-fusion' ) . '</span>';
