			...
			if ( empty( wp_remote_retrieve_body( $response ) ) ) {
				...
				if ( ! empty( $body->code ) ) {
					...
					if ( 'rest_no_route' == $body->code ) {
						...
						$body->message .= ' <strong>' . __( 'This could mean the FunnelKit Automations Pro plugin isn\'t active.', 'wp-fusion' ) . '</strong>';
						$body->message .= ' ' . __( 'Try again or <a href="http://wpfusion.com/contact">contact support</a>.', 'wp-fusion' ) . ' (URL: ' . $url . ')';
					...
					if ( ! empty( $body ) && false !== strpos( $body, 'Enable JavaScript' ) ) {
						$message .= '. ' . __( 'The API request is being blocked by a CloudFlare challenge page.', 'wp-fusion' );
					}
