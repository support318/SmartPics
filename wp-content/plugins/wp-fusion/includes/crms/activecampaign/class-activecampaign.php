...
class WPF_ActiveCampaign {
	...
	public function handle_http_response( $response, $args, $url ) {
		...
		if ( 'WP Fusion; ' . home_url() === $args['user-agent'] && false !== strpos( $url, '.api-' ) ) {
			...
			if ( isset( $body->errors ) ) {
				...
				$response = new WP_Error( 'error', sprintf( __( 'An error has occurred in the API server. [error %d]', 'wp-fusion' ), $code ) );
	...
	public function sync_lists() {
		...
		while ( $proceed ) {
			...
			foreach ( $response->lists as $list ) {
				...
				$available_lists[ $list->id . '_resubscribe' ] = $list->name . ' ' . __( '(resubscribe)', 'wp-fusion' );
			}
		...
		if ( ! empty( $account_fields ) ) {
			...
			if ( ! empty( $response->contact->orgid ) &&
				...
				wpf_log(
					...
					sprintf( __( 'Account fields were provided for contact #%d but no account name was specified, and the contact is not linked to an existing account. Account details were not updated.', 'wp-fusion' ), $contact_id ),
	...
	public function create_account( $account_fields ) {
		...
		wpf_log(
			'info',
			0,
			__( 'Creating new account with data:', 'wp-fusion' ),
			array(
	...
	public function update_account( $account_id, $account_fields ) {
		...
		wpf_log(
			'info',
			0,
			__( 'Updating account with data:', 'wp-fusion' ),
			array(
				'meta_array_nofilter' => $data,
				'source'              => 'activecampaign',
			)
	...
	public function link_account_to_contact( $account_id, $contact_id ) {
		...
		wpf_log(
			...
			sprintf(
				...
				__( 'Linking contact #%1$d to account #%2$d', 'wp-fusion' ),
				$contact_id,
				$account_id
			),
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( false !== strpos( $response->get_error_message(), 'This contact is already associated with an account.' ) ) {
				...
				wpf_log(
					...
					sprintf(
						// translators: %1$d is the contact ID, %2$d is the account ID.
						__( 'The contact #%1$d was previously associated with account #%2$d, deleting existing association and attempting to link again to account #%3$d.', 'wp-fusion' ),
						$contact_id,
