...
class WPF_EmailOctopus_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( $settings['connection_configured'] && wpf_get_option( 'crm' ) === $this->slug ) {
			...
			$new_settings['eo_default_list'] = array(
				'title'       => __( 'Email Octopus List', 'wp-fusion' ),
				'desc'        => __( 'Select a list to use for WP Fusion. If you change the list, you\'ll need to click Refresh Available Tags & Fields (below) to update the available dropdown options.', 'wp-fusion' ),
				'type'        => 'select',
		...
		$new_settings[ "{$this->slug}_api_key" ] = array(
			...
			'desc'        => __( 'You can generate an API key for WP Fusion by going <a target="_blank" href="https://emailoctopus.com/api-key/create">this link</a>.', 'wp-fusion' ),
			'type'        => 'api_validate',
			'section'     => 'setup',
			'class'       => 'api_key',
	...
	public function add_default_fields( $options ) {
		...
		if ( $options['connection_configured'] ) {
			...
			require_once __DIR__ . '/emailoctopus-fields.php';
