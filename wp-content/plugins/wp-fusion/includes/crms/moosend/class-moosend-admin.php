...
class WPF_MooSend_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['moosend_api_key'] = array(
			...
			'desc'        => __( 'Your API key can be found in your MooSend account under Settings.', 'wp-fusion' ),
			'section'     => 'setup',
			'class'       => 'api_key',
		...
		if ( $settings['connection_configured'] && wpf_get_option( 'crm' ) === $this->slug ) {
			...
			$new_settings['moosend_default_list'] = array(
				'title'       => __( 'MooSend Mailing List', 'wp-fusion' ),
				'desc'        => __( 'Select a mailing list to use for WP Fusion. If you change the mailing list, you\'ll need to click Refresh Available Tags & Fields (above) to update the available dropdown options.', 'wp-fusion' ),
				'type'        => 'select',
