...
class WPF_HighLevel_Admin {
	...
	public function oauth_warning() {
		...
		if ( ! wpf_get_option( 'highlevel_token' ) ) {
			...
			echo wp_kses_post( sprintf( __( '<strong>Heads up:</strong> WP Fusion\'s HighLevel integration has been updated to use OAuth authentication. Please %1$sclick here to re-authorize the connection%2$s and enable a deeper integration with new HighLevel features.', 'wp-fusion' ), '<a href="' . $this->get_oauth_url() . '">', '</a>' ) );
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$name = wpf_get_option( 'connection_configured' ) ? wp_fusion()->crm->name : __( 'HighLevel', 'wp-fusion' ); // allows white-labelling.

		$new_settings['highlevel_header'] = array(
			'title'   => sprintf( __( '%s Configuration', 'wp-fusion' ), $name ),
			'type'    => 'heading',
			'section' => 'setup',
		);
		...
		if ( has_filter( 'wpf_get_setting_highlevel_api_key' ) ) {
			...
			$new_settings['highlevel_auth'] = array(
				...
				'desc'    => sprintf( __( 'You\'ll be taken to %1$s to authorize WP Fusion and generate access keys for this site.<br /><br />If you receive authorization errors, please %2$slog in to %1$s%3$s before attempting the connection.', 'wp-fusion' ), $this->crm->name, '<a href="https://app.gohighlevel.com/">', '</a>' ),
			);
			...
			if ( wpf_get_option( 'highlevel_locations' ) ) {
				...
				$new_settings['highlevel_location_id']['title']          = __( 'Account Location', 'wp-fusion' );
				$new_settings['highlevel_location_id']['desc']           = __( 'Select the location you want to use for this site.', 'wp-fusion' );
