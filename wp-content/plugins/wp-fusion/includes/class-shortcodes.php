...
class WPF_Shortcodes {
	...
	public function shortcodes( $atts, $content = '' ) {
		...
		if ( false !== strpos( $atts['tag'], '“' ) || false !== strpos( $atts['not'], '“' ) ) {
			...
			return '<pre>' . sprintf( __( 'Oops! Curly quotes were found in a shortcode parameter of the %s shortcode. Curly quotes do not work with shortcode attributes.', 'wp-fusion' ), '[wpf]' ) . '</pre>';
		}
	...
	public function shortcode_user_meta( $atts, $content = null ) {
		...
		if ( false !== strpos( $atts['field'], '“' ) || false !== strpos( $atts['format'], '“' ) ) {
			// translators: %s is the name of the shortcode.
			return '<pre>' . sprintf( __( 'Oops! Curly quotes were found in a shortcode parameter of the %s shortcode. Curly quotes do not work with shortcode attributes.', 'wp-fusion' ), '[user_meta]' ) . '</pre>';
		}
	...
	public function shortcode_user_meta_if( $atts, $content = null ) {
		...
		foreach ( $atts as $att ) {
			...
			if ( false !== strpos( $att, '“' ) ) {
				// translators: %s is the name of the shortcode.
				return '<pre>' . sprintf( __( 'Oops! Curly quotes were found in a shortcode parameter of the %s shortcode. Curly quotes do not work with shortcode attributes.', 'wp-fusion' ), '[usermeta_if]' ) . '</pre>';
			}
		}
		...
		if ( 'strtotime' === $atts['field_format'] && false === $meta_value ) {
			return sprintf( wp_kses_post( 'Oops! Your input string to the <code>%s</code> attribute was not successfully <a href="https://www.php.net/manual/en/function.strtotime.php" target="_blank">parsed by <code>strtotime()</code></a>.', 'wp-fusion' ), $atts['field'] );
