...
/**
 * Class User_Cancelled_Subscription
 *
 * Handles subscription cancellation triggers with backwards compatibility support.
 ...
 * - Original trigger code (WCSUBSCRIPTIONCANCELLED) and meta (WOOSUBSCRIPTIONS) are preserved
 *
 * @package Uncanny_Automator_Pro\Integrations\Woocommerce_Subscription\Triggers
 */
class User_Cancelled_Subscription extends Trigger {
	 ...
	 * @var string
	 */
	const TRIGGER_CODE = 'WCSUBSCRIPTIONCANCELLED';
	...
	public function setup_trigger() {
		...
		$this->unique_code   = 'WOOSUBSCRIPTIONS';
		$this->storage       = new Subscription_Storage();
		$this->common_tokens = new Common_Tokens( $this->unique_code );
		$this->order_tokens  = new Order_Tokens();

		// Dropdown label.
		$this->set_readable_sentence(
			/* translators: %1$s: Subscription product */
			esc_html_x( 'A user cancels a subscription to {{a product}}', 'WooCommerce Subscription', 'uncanny-automator-pro' )
		);
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html_x( 'A user cancels a subscription to {{a product:%1$s}} {{a number of:%2$s:%1$s}} times', 'WooCommerce Subscription', 'uncanny-automator-pro' ),
				self::TRIGGER_META,
				'NUMTIMES'
			)
		);
		...
		$this->add_action( 'woocommerce_subscription_status_cancelled', 30, 2 );
	...
	public function options() {
		...
		$field_subscriptions = array(
			'option_code'     => $this->unique_code, // Retained for backwards compatibility.
	...
	public function validate( $trigger, $hook_args ) {
		...
		$selected_subscription_id = intval( $trigger['meta']['WOOSUBSCRIPTIONS'] ?? 0 );
