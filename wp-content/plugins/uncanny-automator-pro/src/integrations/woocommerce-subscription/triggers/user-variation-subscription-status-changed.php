...
/**
 * Class User_Variation_Subscription_Status_Changed
 *
 * Handles variation subscription status change triggers with backwards compatibility support.
 * This class maintains compatibility with existing automations while providing enhanced token support.
 *
 * Backwards Compatibility Notes:
 * - Maintains original trigger code (WCVARIATIONSUBSCRIPTIONSTATUSCHANGED) and meta (WOOSUBSCRIPTIONS, WOOSUBSCRIPTIONSTATUS)
 ...
 */
class User_Variation_Subscription_Status_Changed extends Trigger {
	 ...
	 * @var string
	 */
	const TRIGGER_CODE = 'WCVARIATIONSUBSCRIPTIONSTATUSCHANGED';
	...
	/**
	 * Product variation tokens
	 *
	 * @var Variation_Product_Tokens
	 */
	protected $variation_tokens;
	...
	public function setup_trigger() {
		...
		$this->variation_tokens     = new Variation_Product_Tokens( self::TRIGGER_META );
		$this->product_extra_tokens = new Product_Extra_Tokens();

		// Dropdown label.
		$this->set_readable_sentence(
			/* translators: %1$s: Variation, %2$s: Variable subscription, %3$s: Status */
			esc_html_x( "A user's subscription to {{a specific variation}} of {{a variable subscription}} is set to {{a status}}", 'WooCommerce Subscription', 'uncanny-automator-pro' )
		);
		...
		$this->set_sentence(
			sprintf(
				/* translators: %1$s: Variation, %2$s: Variable subscription, %3$s: Status */
				esc_html_x( "A user's subscription to {{a specific variation:%1\$s}} of {{a variable subscription:%2\$s}} is set to {{a status:%3\$s}}", 'WooCommerce Subscription', 'uncanny-automator-pro' ),
	...
	public function define_tokens( $trigger, $tokens ) {
		...
		$variation_tokens     = $this->variation_tokens->get_tokens();
		$product_extra_tokens = $this->product_extra_tokens->get_tokens();

		// Add variation tokens to the tokens array.
		$variation_token = array(
			'tokenId'   => 'WOOVARIPRODUCT',
			'tokenName' => esc_html_x( 'Variation', 'WooCommerce Subscription', 'uncanny-automator-pro' ),
			'tokenType' => 'text',
		);

		// Add the status. (this is a legacy token)
		$status_token = array(
			'tokenId'   => 'WOOSUBSCRIPTIONSTATUS',
			'tokenName' => esc_html_x( 'Subscription status (legacy)', 'WooCommerce Subscription', 'uncanny-automator-pro' ),
			'tokenType' => 'text',
		);

		$this->common_tokens->add_token( $variation_token );
		$this->common_tokens->add_token( $status_token );
	...
	public function hydrate_tokens( $trigger, $hook_args ) {
		...
		$variation_product     = $subscription_provider->get_subscription_variation();
		...
		$variation_product_tokens = $this->variation_tokens->get_tokens();
		foreach ( $variation_product_tokens as $token ) {
			$tokens[ $token['tokenId'] ] = $this->variation_tokens->parse_token( $token['tokenId'], $subscription );
		}
		...
		// Store the woo variation product tokens. Another backwards compatibility.
	...
	public function validate( $trigger, $hook_args ) {
		...
		if ( 0 === $selected_parent_id || 0 === $selected_variation_id || empty( $selected_status ) ) {
			return false;
		}
