 ...
 * Studiocart Trigger
 */
class STUDIOCART_SUBSCRIPTIONCANCELLEDPRODUCT {
	...
	public function __construct() {
		$this->trigger_code = 'SUBSCRIPTIONCANCELLEDPRODUCT';
		$this->trigger_meta = 'STUDIOCARTSUBS';
		$this->define_trigger();
	}
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( "A user's subscription is cancelled for {{a product:%1\$s}}", 'uncanny-automator-pro' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Studiocart */
			'select_option_name'  => esc_attr__( "A user's subscription is cancelled for {{a product}}", 'uncanny-automator-pro' ),
			'action'              => 'sc_subscription_canceled',
			'priority'            => 100,
			'accepted_args'       => 3,
			'validation_function' => array( $this, 'sc_subscription_canceled' ),
	 ...
	 */
	public function sc_subscription_canceled( $status, $order_data, $order_type ) {
				...
				foreach ( $args as $result ) {
					if ( true === $result['result'] ) {
						...
						$pass_args = array(
							...
							'meta_value'     => $order_data['first_order'],
