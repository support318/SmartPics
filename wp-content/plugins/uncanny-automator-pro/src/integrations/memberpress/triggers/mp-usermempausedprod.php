...
class MP_USERMEMPAUSEDPROD {
	 ...
	 */
	public function mp_product_paused( $event ) {

		$sub        = $event->get_data();
		$product_id = absint( $sub->product_id );
	...
	public function maybe_migrate_action_hook() {
		...
		if ( ! empty( $post_ids ) ) {
			...
			$sql          = $wpdb->prepare(
				...
				array_merge( array( 'mepr-event-subscription-paused', 'mepr_subscription_transition_status', 'add_action' ), $post_ids )
			);
			$wpdb->query( $sql );
		}
