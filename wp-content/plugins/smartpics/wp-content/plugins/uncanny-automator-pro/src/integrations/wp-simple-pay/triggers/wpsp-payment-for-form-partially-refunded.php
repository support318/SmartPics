 ...
 */
class WPSP_PAYMENT_FOR_FORM_PARTIALLY_REFUNDED extends Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'WPSP_PAYMENT_PARTIALLY_REFUNDED' );
		$this->set_trigger_meta( 'WPSPFORMS' );
		$this->set_trigger_type( 'anonymous' );
		$this->set_is_pro( true );
		$this->set_sentence(
			// translators: %s is a WP Simple Pay form
			sprintf( esc_attr_x( 'A payment for {{a form:%1$s}} is partially refunded', 'WP Simple Pay', 'uncanny-automator-pro' ), $this->get_trigger_meta() )
		);
		$this->set_readable_sentence( esc_attr_x( 'A payment for {{a form}} is partially refunded', 'WP Simple Pay', 'uncanny-automator-pro' ) );
	...
	public function validate( $trigger, $hook_args ) {
		...
		return ( ( intval( '-1' ) === intval( $selected_form_id ) || absint( $selected_form_id ) === absint( $form_id ) ) && $refunded_amount < $original_amount );
	}
	...
	public function define_tokens( $trigger, $tokens ) {
		...
		$trigger_tokens      = array(
			...
			array(
				...
				'tokenName' => esc_html_x( 'Refunded amount', 'Wp Simple Pay', 'uncanny-automator-pro' ),
				'tokenType' => 'text',
			),
		);
