...
class BP_USERREGISTERWITHFIELD {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'migrate_bp_trigger_hook' ), 99 );
		$this->define_trigger();
	}
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr_x( 'A user registers with {{a specific value:%1$s}} in {{a specific field:%2$s}}', 'BuddyPress', 'uncanny-automator-pro' ), 'SUBVALUE:' . $this->trigger_meta, $this->trigger_meta ),
			/* translators: Logged-in trigger - BuddyPress */
			'select_option_name'  => esc_attr_x( 'A user registers with {{a specific value}} in {{a specific field}}', 'BuddyPress', 'uncanny-automator-pro' ),
			...
			array(
				'options_group' => array(
					$this->trigger_meta => array(
						...
						Automator()->helpers->recipe->field->text_field( 'SUBVALUE', esc_attr_x( 'Value', 'BuddyPress', 'uncanny-automator-pro' ) ),
					),
				),
			)
		);
	...
	/**
	 * Migrate the BP trigger hook to include additional action hooks.
	 *
	 * @return void
	 */
	public function migrate_bp_trigger_hook() {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				if ( false !== $updated_count && $updated_count > 0 ) {
					automator_log(
						sprintf(
							'BP trigger migration completed. Updated %d trigger(s) with new action hooks.',
							$updated_count
						),
						$this->trigger_code
					);
