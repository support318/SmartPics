...
/**
 * Class FLUENTCOMMUNITY_USER_SPACE_REQUESTED
 *
 * @package Uncanny_Automator
 */
class FLUENTCOMMUNITY_USER_SPACE_REQUESTED extends Trigger {

	protected $prefix = 'FLUENTCOMMUNITY_USER_SPACE_REQUESTED';
	...
	protected function setup_trigger() {
		...
		$this->add_action( 'fluent_community/space/join_requested' );
		$this->set_action_args_count( 2 );

		$this->set_sentence(
			sprintf(
				/* translators: %1$s - Space */
				esc_html_x( 'A user submits a request to join {{a space:%1$s}}', 'FluentCommunity', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_html_x( 'A user submits a request to join {{a space}}', 'FluentCommunity', 'uncanny-automator' )
		);
	}
	/**
	 * Options.
