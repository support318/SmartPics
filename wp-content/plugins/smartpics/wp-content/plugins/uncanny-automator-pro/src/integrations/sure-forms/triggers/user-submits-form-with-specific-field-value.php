...
/**
 * Class USER_SUBMITS_FORM_WITH_SPECIFIC_FIELD_VALUE
 *
 * @package Uncanny_Automator_Pro
 */
class USER_SUBMITS_FORM_WITH_SPECIFIC_FIELD_VALUE extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'USER_SUBMIT_FORM_WITH_SPECIFIC_FIELD' );
		$this->set_trigger_meta( 'SURE_FORMS' );
		$this->set_is_pro( true );
		$this->set_sentence(
			// translators: %1$s: Form, %2$s: Value, %3$s: Field
			sprintf( esc_attr_x( 'A user submits {{a form:%1$s}} with {{a specific value:%2$s}} in {{a specific field:%3$s}}', 'Sure Forms', 'uncanny-automator-pro' ), $this->get_trigger_meta(), 'SURE_VALUE:' . $this->get_trigger_meta(), 'SURE_FIELD:' . $this->get_trigger_meta() )
		);
		$this->set_readable_sentence(
			// translators: Trigger sentence - Sure Forms
			esc_attr_x( 'A user submits {{a form}} with {{a specific value}} in {{a specific field}}', 'Sure Forms', 'uncanny-automator-pro' )
		);
