 ...
 */
class Thrive_Apprentice_Product_User_Access_Grant extends Action {
	...
	protected function setup_action() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr_x( 'Grant the user access to {{a product:%1$s}}', 'Thrive Apprentice', 'uncanny-automator-pro' ),
				$this->get_action_meta()
			)
		);

		$this->set_readable_sentence( esc_attr_x( 'Grant the user access to {{a product}}', 'Thrive Apprentice', 'uncanny-automator-pro' ) );
	}
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		return Automator()->complete->action( $user_id, $action_data, $recipe_id, esc_html_x( 'Thrive Apprentice plugin must be installed and activated for this action to run.', 'Thrive Apprentice', 'uncanny-automator-pro' ) );
	}
	...
	private function is_dependencies_loaded() {
		...
		return class_exists( '\TVA_Customer' )
			&& method_exists( '\TVA_Customer', 'enrol_user_to_product' );
	}
}
