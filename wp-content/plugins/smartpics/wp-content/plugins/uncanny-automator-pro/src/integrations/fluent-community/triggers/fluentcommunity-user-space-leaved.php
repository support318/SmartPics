...
/**
 * Class FLUENTCOMMUNITY_USER_SPACE_LEAVED
 *
 * @package Uncanny_Automator
 */
class FLUENTCOMMUNITY_USER_SPACE_LEAVED extends Trigger {

	protected $prefix = 'FLUENTCOMMUNITY_USER_SPACE_LEAVED';
	...
	protected function setup_trigger() {
		...
		$this->add_action( 'fluent_community/space/user_left' );
		$this->set_action_args_count( 3 );

		$this->set_sentence(
			sprintf(
				/* translators: %1$s - Space */
				esc_html_x( 'A user leaves {{a space:%1$s}}', 'FluentCommunity', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_html_x( 'A user leaves {{a space}}', 'FluentCommunity', 'uncanny-automator' )
		);
	}
	/**
	 * Options.
