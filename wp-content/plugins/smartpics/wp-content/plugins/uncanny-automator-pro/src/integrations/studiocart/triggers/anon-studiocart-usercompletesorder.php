 ...
 * Studiocart Trigger
 */
class ANON_STUDIOCART_USERCOMPLETESORDER {
	...
	public function __construct() {
		$this->trigger_code = 'ANONUSERCOMPLETESORDER';
		$this->trigger_meta = 'STUDIOCARTORDERGUEST';
		$this->define_trigger();
	}
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( 'A guest completes an order for {{a product:%1$s}}', 'uncanny-automator-pro' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Studiocart */
			'select_option_name'  => esc_attr__( 'A guest completes an order for {{a product}}', 'uncanny-automator-pro' ),
			'action'              => 'sc_order_complete',
			'type'                => 'anonymous',
			'priority'            => 100,
			'accepted_args'       => 3,
			'validation_function' => array( $this, 'sc_order_completed' ),
	 ...
	 */
	public function sc_order_completed( $status, $order_data, $order_type ) {
