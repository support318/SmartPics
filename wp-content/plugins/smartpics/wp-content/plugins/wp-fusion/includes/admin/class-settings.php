...
class WPF_Settings {
	...
	public function add_admin_bar_item( WP_Admin_Bar $wp_admin_bar ) {
		...
		$wp_admin_bar->add_menu(
			array(
				'parent' => 'wpfusion',
				'title'  => __( 'Refresh Available Tags & Fields', 'wp-fusion' ),
	...
	public function init() {
		...
		if ( wp_fusion()->is_full_version() ) {
			...
			add_action( 'in_plugin_update_message-wp-fusion/wp-fusion.php', array( $this, 'update_available_notice' ), 10, 2 );
	...
	public function add_action_links( $links ) {
		...
		$links[] = '<a href="' . esc_url( get_admin_url( null, 'options-general.php?page=wpf-settings' ) ) . '">' . esc_html__( 'Settings', 'wp-fusion' ) . '</a>';
	...
	public function add_row_links( $links, $file ) {
		...
		if ( WPF_PLUGIN_PATH === $file ) {
			...
			$links[] = sprintf( '<a href="https://wpfusion.com/documentation/faq/changelog/" target="_blank" rel="noopener">%s</a>', esc_html__( 'View changelog', 'wp-fusion' ) );
		...
		$localize = array(
			...
			'strings'    => array(
				...
				'webhooks'                  => array(
					...
					'success'         => sprintf( __( 'Success! Your site is able to receive incoming webhooks. Note that this means your site is not blocking webhooks from wpfusion.com, it is still possible for your server to block webhooks from %s.', 'wp-fusion' ), wp_fusion()->crm->name ),
				...
				'fluentcrmTagFormatWarning' => __( 'After changing the tag format, you will need to run a Resync Tags for Every User operation from the Advanced settings tab to load each user\'s tags in the new format.', 'wp-fusion' ),
			),
		);
	...
	public function set_tag_labels( $translation, $text, $domain ) {
		...
		if ( $domain && 0 === strpos( $domain, 'wp-fusion' ) ) {
	...
	public function add_tag() {
		...
		if ( empty( $_POST['tag'] ) || empty( $_POST['tag']['id'] ) ) {
			wp_send_json_error( new WP_Error( 'error', __( 'Tag name is empty!.', 'wp-fusion' ) ) );
		}
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							__( 'Your license key expired on %s.', 'wp-fusion' ),
					...
					case 'disabled':
					case 'revoked':
						$message = __( 'Your license key has been disabled.', 'wp-fusion' );
						break;

					case 'missing':
						$message = __( 'Invalid license.', 'wp-fusion' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = __( 'Your license is not active for this URL.', 'wp-fusion' );
						break;

					case 'item_name_mismatch':
						/* translators: the plugin name */
						$message = sprintf( __( 'This appears to be an invalid license key for WP Fusion.', 'wp-fusion' ) );
						break;

					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.', 'wp-fusion' );
						break;

					default:
						$message = __( 'An error occurred, please try again.', 'wp-fusion' );
						break;
				}
	...
	public function update_available_notice( $plugin_data, $response ) {
		...
		if ( empty( $response->package ) ) {
			$license_page        = admin_url( 'options-general.php?page=wpf-settings#setup' );
			...
			printf(
				...
				/* translators: 1: licence key link start, 2: license key link end */
					__( 'Activate %1$syour license key%2$s to enable updates.', 'wp-fusion' ),
					$settings_link_open,
					$settings_link_close
				)
			);
	...
	public function configure_sections( $page ) {
		...
		$sections['main']           = __( 'General Settings', 'wp-fusion' );
		$sections['contact-fields'] = __( 'Contact Fields', 'wp-fusion' );

		if ( wp_fusion()->is_full_version() ) {
			$sections['integrations'] = __( 'Integrations', 'wp-fusion' );
		}

		$sections['import']   = __( 'Import Users', 'wp-fusion' );
		$sections['setup']    = __( 'Setup', 'wp-fusion' );
		$sections['advanced'] = __( 'Advanced', 'wp-fusion' );
	...
	public function get_settings( $settings, $options ) {
		...
		if ( wp_fusion()->crm->supports( 'lists' ) ) {
			...
			$settings['assign_lists'] = array(
				'title'       => __( 'Assign Lists', 'wp-fusion' ),
				'desc'        => __( 'All new user registrations will be added to the selected list(s).', 'wp-fusion' ),
				'type'        => 'multi_select',
		...
		*/
		...
		$settings['login_sync'] = array(
			...
			'tooltip' => sprintf( __( 'Note: this is only necessary if you are applying tags via automations in %s and haven\'t set up webhooks to send the data back. Any tags applied via WP Fusion are available in WordPress immediately.', 'wp-fusion' ), wp_fusion()->crm->name ),
		);
		...
		$settings['login_meta_sync'] = array(
			...
			'section' => 'main',
			// translators: %s is the name of the CRM.
			'tooltip' => sprintf( __( 'Note: this is only necessary if you are manually updating contact data in %s and haven\'t set up webhooks to send the data back.', 'wp-fusion' ), wp_fusion()->crm->name ),
		);
		...
		*/
		...
		$settings['restrict_content'] = array(
			'title'   => __( 'Restrict Content', 'wp-fusion' ),
			'desc'    => __( 'Enable WP Fusion\'s access control system.', 'wp-fusion' ),
			'std'     => 1,
			'type'    => 'checkbox',
			'section' => 'main',
			'unlock'  => array(
			...
			'tooltip' => sprintf( __( 'WP Fusion includes many features for restricting access to content based on tags in %1$s. However if you are not using these features (you only want to sync data with %2$s), you can un-check this setting to disable the access control interfaces and features.', 'wp-fusion' ), wp_fusion()->crm->name, wp_fusion()->crm->name ),
		);
		...
		$settings['return_after_login'] = array(
			...
			'desc'    => __( 'If a user has tried to view a restricted page, take them back to that page after logging in.', 'wp-fusion' ),
		...
		$settings['default_not_logged_in_redirect'] = array(
			'title'   => __( 'Default Not Logged-In Redirect', 'wp-fusion' ),
			'desc'    => __( 'If you specify a URL here (usualy the URL to your login page), it will take priority over all other redirects in cases where the user is denied access and is not logged in.', 'wp-fusion' ),
			'type'    => 'text',
			'section' => 'main',
		);

		$settings['default_redirect'] = array(
			'title'   => __( 'Default Generic Redirect', 'wp-fusion' ),
		...
		$settings['restricted_message'] = array(
			...
			'desc'          => sprintf( __( '%1$sRestricted content message%2$s for when a redirect isn\'t set. Use <code>[the_excerpt]</code> or <code>[the_excerpt length="120"]</code> to display an excerpt of the restricted content.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/getting-started/access-control/#restricted-content-message" target="_blank">', '</a>' ),
		...
		*/
		...
		$settings['site_lockout_header'] = array(
			...
			'type'    => 'heading',
			'section' => 'main',
			'desc'    => __( 'Site lockout lets you restrict access to all pages on your site if a user has a specific tag (i.e. "Payment Failed"). The only pages accessible will be the URL specified as the Lockout Redirect below, and any additional URLs in the Allowed URLs setting.', 'wp-fusion' ),
		);
		...
		$settings['lockout_redirect'] = array(
			...
			'desc'    => __( 'URL to redirect to when lockout is active. (Must be a full URL)', 'wp-fusion' ),
			'std'     => wp_login_url(),
			'type'    => 'text',
			'section' => 'main',
		);
		...
		$settings['lockout_allowed_urls'] = array(
			...
			'desc'        => __( 'The URLs listed here (one per line) will bypass the Site Lockout feature. You can use a wildcard * for partial matches.', 'wp-fusion' ),
			'type'        => 'textarea',
			'section'     => 'main',
			'rows'        => 3,
		...
		*/
		...
		$settings['send_welcome_email'] = array(
			'title'   => __( 'Send Welcome Email', 'wp-fusion' ),
			'desc'    => sprintf( __( 'Send the WordPress default new user welcome email, with a password reset link.', 'wp-fusion' ), wp_fusion()->crm->name ),
			'type'    => 'checkbox',
			'section' => 'main',
		);
		...
		*/
		...
		if ( in_array( 'leads', wp_fusion()->crm->supports ) ) {
			...
			$settings['leads_header'] = array(
				'title'   => __( 'Leads', 'wp-fusion' ),
				'type'    => 'heading',
				'section' => 'integrations',
			);

			$settings['leads'] = array(
				'title'   => __( 'Sync Leads', 'wp-fusion' ),
				'desc'    => sprintf( __( 'Sync guest %1$sform submissions to Leads%2$s in %3$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/crm-specific-docs/updating-leads/" target="_blank">', '</a>', wp_fusion()->crm->name ),
				'type'    => 'checkbox',
				'section' => 'integrations',
				'tooltip' => sprintf( __( 'When this setting is enabled, form submissions will first check for a contact record by email address. If no contact is found, a lead will be created, and any tags will be applied to the lead. If the user is logged in and already has a contact record, the existing contact will be used instead.', 'wp-fusion' ), wp_fusion()->crm->name ),
			);
		...
		*/
		...
		$settings['update_existing_users'] = array(
			'title'   => __( 'Update Existing Users', 'wp-fusion' ),
			'desc'    => __( 'Also load the tags and metadata for contacts who already have user accounts.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'import',
		);
		...
		*/
		...
		$settings['deletion_tags'] = array(
			...
			'desc'    => sprintf( __( 'These tags will be applied in %s when a user is deleted from the site, or when a user deletes their own account.', 'wp-fusion' ), wp_fusion()->crm->name ),
			'std'     => array(),
			'type'    => 'assign_tags',
			'section' => 'advanced',
		);
		...
		$settings['link_click_tracking'] = array(
			...
			'desc'    => __( 'Enqueue the script to handle link click tracking. See <a href="https://wpfusion.com/documentation/tutorials/link-click-tracking/" target="_blank">this tutorial</a> for more info.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		);

		$settings['js_leadsource_tracking'] = array(
			'title'   => __( 'JS Lead Source Tracking', 'wp-fusion' ),
			'desc'    => __( 'Enqueue a script to handle setting the <a href="https://wpfusion.com/documentation/tutorials/lead-source-tracking/" target="_blank">lead source tracking cookies</a>.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
			'tooltip' => __( 'This fixes tracking on WP Engine, Flywheel, and other hosts that sanitize UTM parameters out of URLs.', 'wp-fusion' ),
		);

		$settings['tags_as_classes'] = array(
			'title'   => __( 'Tags as CSS Classes', 'wp-fusion' ),
			'desc'    => __( 'Add the current user\'s tags <a href="https://wpfusion.com/documentation/getting-started/access-control/#protecting-content-via-css" target="_blank">as CSS classes</a> to the HTML <code>&lt;body&gt;</code> element of each page.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		);
		...
		$settings['staging_mode'] = array(
			...
			'desc'    => sprintf( __( 'When <a href="https://wpfusion.com/documentation/faq/staging-sites/" target="_blank">staging mode</a> is active, all normal WP Fusion features will be available, but no API calls will be made to %s, and no tags or access to content will be modified.', 'wp-fusion' ), wp_fusion()->crm->name ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		);
		...
		if ( defined( 'WPF_STAGING_MODE' ) && true === WPF_STAGING_MODE ) {
			...
			$settings['staging_mode']['desc']     = __( 'Staging mode is enabled in wp-config.php, via the WPF_STAGING_MODE constant.', 'wp-fusion' );
		}

		if ( WPF_Staging_Sites::is_duplicate_site() ) {
			$settings['staging_mode']['disabled'] = true;
			$settings['staging_mode']['desc']     = __( 'To disable staging mode, please click Disable Staging Mode at the top of this page.', 'wp-fusion' );
		}
		...
		$settings['enable_cron'] = array(
			'title'   => __( 'Enable Cron', 'wp-fusion' ),
			'desc'    => __( 'Enables a cron task to start the background worker and check if any tasks need to be performed (usually used with <a href="https://wpfusion.com/documentation/other-common-issues/webhooks-not-being-received-by-wp-fusion/#the-async-method" target="_blank">asynchronous webhooks</a>).', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		...
		$settings['cron_interval'] = array(
			'title'   => __( 'Cron Interval', 'wp-fusion' ),
			'desc'    => __( 'How often the cron job will run (in minutes).', 'wp-fusion' ),
			'std'     => 5,
			'type'    => 'number',
			'section' => 'advanced',
		);
		...
		$settings['logging_badge'] = array(
			'title'   => __( 'Show Badge', 'wp-fusion' ),
			'desc'    => __( 'Show a notification badge on the Tools menu in the admin when there are unseen log errors.', 'wp-fusion' ),
			'std'     => 1,
			'type'    => 'checkbox',
			'section' => 'advanced',
		);
		...
		$settings['enable_admin_bar'] = array(
			...
			'desc'    => __( 'Enable the "Preview With Tag" and "Refresh Tags" functionality on the admin bar.', 'wp-fusion' ),
		...
		$settings['status_header'] = array(
			'title'   => __( 'Status', 'wp-fusion' ),
			'type'    => 'heading',
			'section' => 'advanced',
		);

		$settings['status_hooks'] = array(
			'title'   => __( 'Custom Code', 'wp-fusion' ),
			'type'    => 'status_hooks',
			'section' => 'advanced',
		);
		...
		$settings['reset'] = array(
			'title'   => __( 'Reset', 'wp-fusion' ),
			'desc'    => __( 'Check this box and click "Save Changes" below to reset the options to their defaults.', 'wp-fusion' ),
			'tooltip' => __( 'This will reset the options on this settings page, allowing you to connect to a new CRM. It does not remove the access rules on your content, or other settings saved in different parts of the database.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		...
		$settings['reset_options'] = array(
			'title'   => __( 'Reset All', 'wp-fusion' ),
			'desc'    => __( 'Also erase all WP Fusion settings configured on posts, pages, courses, memberships, and elsewhere.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'advanced',
		);

		$settings['import_export_header'] = array(
			'title'   => __( 'Import / Export Settings', 'wp-fusion' ),
			'type'    => 'heading',
			'section' => 'advanced',
		);

		$settings['export_settings'] = array(
			'title'   => __( 'Export Settings', 'wp-fusion' ),
			'type'    => 'export_settings',
			'section' => 'advanced',
		);

		$settings['import_settings'] = array(
			'title'   => __( 'Import Settings', 'wp-fusion' ),
	...
	public function show_field_api_validate( $id, $field ) {
		...
		if ( ! is_scalar( $value ) ) {
			...
			echo '<strong>' . esc_html__( 'Error: The API key is not a string. Please reauthorize the connection.', 'wp-fusion' ) . '</strong>';
			echo '<pre>';
			print_r( $value );
			echo '</pre>';
			$value = '';
		...
		if ( $this->connection_configured ) {
			...
			echo '<span class="dashicons dashicons-update-alt"></span>';
			echo '<span class="text">' . esc_html__( 'Connect', 'wp-fusion' ) . '</span>';
			echo '</a>';
	...
	public function show_field_oauth_authorize( $id, $field ) {
		...
		echo '<a id="' . esc_attr( $field['slug'] ) . '-auth-btn" class="button ' . ( isset( $field['dis'] ) ? 'button-disabled' : 'button-primary' ) . '" href="' . esc_url( $field['url'] ) . '">' . sprintf( esc_html__( 'Authorize with %s', 'wp-fusion' ), esc_html( $field['name'] ) ) . '</a><br />';

		if ( empty( $field['desc'] ) ) {
			// Translators: %s is the name of the CRM.
			echo '<span class="description">' . sprintf( esc_html__( 'You\'ll be taken to %s to authorize WP Fusion and generate access keys for this site.', 'wp-fusion' ), esc_html( $field['name'] ) ) . '</td>';
		}

		if ( ! is_ssl() ) {
			echo '<p class="wpf-notice notice notice-error">' . sprintf( esc_html__( '<strong>Warning:</strong> Your site is not currently SSL secured (https://). You will not be able to connect to the %s API. Your Site Address must be set to https:// in Settings &raquo; General.', 'wp-fusion' ), $field['name'] ) . '</p>';
		}
	}
	...
	public function show_field_edd_license( $id, $field ) {
		...
		if ( 'valid' === $this->license_status ) {
			echo '<a id="edd-license" data-action="edd_deactivate" class="button activated">' . esc_html__( 'Deactivate License', 'wp-fusion' ) . '</a>';
		} else {
			echo '<a id="edd-license" data-action="edd_activate" class="button">' . esc_html__( 'Activate License', 'wp-fusion' ) . '</a>';
		}
		echo '<span class="description">' . esc_html__( 'Enter your license key for automatic updates and support.', 'wp-fusion' ) . '</span>';
	...
	public function show_field_import_users( $id, $field ) {
		...
		echo '<select class="select4 select4-search" id="wpf_options-import_users" name="wpf_options[import_users]" data-placeholder="' . esc_attr__( 'Select a tag', 'wp-fusion' ) . '">';

		echo '<option></option>'; // placeholder.

		echo '<option value="false">' . esc_html__( '(All contacts)', 'wp-fusion' ) . '</option>';
		...
		echo '<select class="select4" id="import_role" data-placeholder="' . esc_attr__( 'Select a user role', 'wp-fusion' ) . '">';
		...
		echo '<a id="import-users-btn" class="button">' . esc_html__( 'Import', 'wp-fusion' ) . '</a>';
	}
	...
	public function show_field_contact_fields( $id, $field ) {
		...
		$field_groups['custom'] = array(
			'title' => __( 'Custom Field Keys (Added Manually)', 'wp-fusion' ),
		);
		...
		echo '<p>' . sprintf( esc_html__( 'For more information on these settings, %1$ssee our documentation%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/getting-started/syncing-contact-fields/" target="_blank">', '</a>' ) . '</p>';
		echo '<br />';
		...
		// echo '<th class="sync">' . esc_html__( 'Pull', 'wp-fusion' ) . '</th>'; @TODO.
		echo '<th>' . esc_html__( 'Name', 'wp-fusion' ) . '</th>';
		...
		foreach ( $field_groups as $group => $group_data ) {
			...
			if ( 'standard-wordpress-fields' !== $group_slug ) { // Skip the first one
				...
				if ( isset( $group_data['url'] ) ) {
					echo '<a class="table-header-docs-link" href="' . esc_url( $group_data['url'] ) . '" target="_blank">' . esc_html__( 'View documentation', 'wp-fusion' ) . ' &rarr;</a>';
				}
			...
			foreach ( $group_data['fields'] as $user_meta => $data ) {
				...
				if ( ! empty( $this->options['custom_metafields'] ) && in_array( $user_meta, $this->options['custom_metafields'] ) ) {
					echo ' (' . esc_html__( 'Added by user', 'wp-fusion' ) . ')';
				}
	...
	public function show_field_import_groups( $id, $field ) {
		...
		foreach ( $import_groups as $date => $data ) {
			...
			// Prior to 3.43.1, some user IDs were being saved twice, and WP_Errors were being saved to the array.
			$data['user_ids'] = array_unique( array_filter( $data['user_ids'], 'is_numeric' ) );
	...
	public function show_field_export_options( $id, $field ) {
		...
		echo '<div class="skip-processed-container">';
			...
			echo '<label for="skip_already_processed">' . esc_html__( 'Skip already processed', 'wp-fusion' ) . ' [placeholder]';
				echo '<i class="fa fa-question-circle wpf-tip" data-tip="' . esc_html__( 'If checked, [placeholder] that have already been successfully processed by WP Fusion will be excluded from the export.', 'wp-fusion' ) . '"></i>';
			echo '</label><br />';
		echo '</div>';

		echo '<br /><a id="export-btn" class="button">' . esc_html__( 'Create Background Task', 'wp-fusion' ) . '</a><br />';
	}
	...
	public function show_field_status_hooks( $id, $field ) {
		...
		if ( ! empty( $hooks ) ) {
			...
			echo '<tr>';
				echo '<th>' . esc_html__( 'Hook', 'wp-fusion' ) . '</th>';
				echo '<th style="width: 32px;">' . esc_html__( 'Priority', 'wp-fusion' ) . '</th>';
				echo '<th style="width: 32px;"></th>';
				echo '<th>' . esc_html__( 'Attached Function', 'wp-fusion' ) . '</th>';
			echo '</tr>';
			...
			foreach ( $hooks as $hook ) {
				...
				if ( is_object( $hook['callback']['function'] ) ) {
					echo '<em>' . esc_html__( 'anonymous function', 'wp-fusion' ) . '</em>';
			...
			echo '<span class="description">' . esc_html__( 'This table lists any custom code that might be modifying the behavior of WP Fusion.', 'wp-fusion' ) . '</span>';

		} else {

			esc_html_e( 'No custom code detected.', 'wp-fusion' );
	...
	public function show_field_test_webhooks( $id, $field ) {
		...
		echo '<a class="button" data-url="' . esc_url( home_url() ) . '" id="test-webhooks-btn" href="#">' . esc_html__( 'Test Webhooks', 'wp-fusion' ) . '</a>';
	}
	...
	public function debug_settings_output() {
		...
		if ( isset( $_GET['debug'] ) ) {
			...
			echo '</pre>';
			echo '<pre>';
			echo 'Import Groups:<br />';
			print_r( get_option( 'wpf_import_groups' ) );
			echo '</pre>';
			echo '</div>';
		}
	}
	...
	public function show_field_export_settings( $id, $field ) {
		echo '<a id="wpf-export-settings" class="button" href="' . esc_url( admin_url( 'admin-ajax.php' ) . '?action=wpf_export_settings&_wpnonce=' . wp_create_nonce( 'wpf_settings_export' ) ) . '">' . esc_html__( 'Export Settings', 'wp-fusion' ) . '</a>';
	}
	...
	public function show_field_import_settings( $id, $field ) {
		...
		echo '<a id="wpf-import-settings-button" class="button" href="' . esc_url( admin_url( 'admin-ajax.php' ) . '?action=wpf_import_settings&_wpnonce=' . wp_create_nonce( 'wpf_settings_import' ) ) . '">' . esc_html__( 'Import Settings', 'wp-fusion' ) . '</a>';
	}
	...
	public function handle_settings_import() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'You do not have permission to perform this action.', 'wp-fusion' ) );
		}

		if ( ! isset( $_FILES['file'] ) ) {
			wp_send_json_error( __( 'No file provided.', 'wp-fusion' ) );
		}
		...
		if ( ! in_array( $file['type'], $file_types ) ) {
			wp_send_json_error( __( 'Invalid file type. Please upload a CSV file.', 'wp-fusion' ) );
		}
		...
		wp_send_json_success( __( 'Settings imported successfully.', 'wp-fusion' ) );
	}
}
