...
class WPF_User_Profile {
	...
	public function add_bulk_actions( $bulk_actions ) {
		$bulk_actions['users_sync']      = __( 'Resync contact IDs and tags', 'wp-fusion' );
		$bulk_actions['users_meta']      = __( 'Push user meta', 'wp-fusion' );
		$bulk_actions['pull_users_meta'] = __( 'Pull user meta', 'wp-fusion' );
		return $bulk_actions;
	}
		...
		if ( 'register' === $action ) {
			...
			if ( ! is_wp_error( $result ) ) {
				...
				$message = sprintf(
					...
					__( '<strong>Success:</strong> User was added to %1$s with contact ID %2$s.', 'wp-fusion' ),
					wp_fusion()->crm->name,
					$result
				);
				...
				$message = sprintf(
					...
					__( '<strong>Error:</strong> Unable to create contact in %1$s: %2$s.', 'wp-fusion' ),
					wp_fusion()->crm->name,
			...
			if ( ! is_wp_error( $result ) ) {
				...
				$message = sprintf(
					/* translators: %1$s CRM name */
					__( '<strong>Success:</strong> Loaded metadata from %1$s:', 'wp-fusion' ),
				...
				$message = sprintf(
					/* translators: %1$s CRM name */
					__( '<strong>Error:</strong> Unable to pull metadata from %1$s: %2$s.', 'wp-fusion' ),
					esc_html( wp_fusion()->crm->name ),
					$result->get_error_message()
				);
			...
			if ( ! is_wp_error( $result ) ) {
				...
				$message = sprintf(
					/* translators: %1$s CRM name, %2$s contact ID */
					__( '<strong>Success:</strong> Synced user meta to %1$s contact ID %2$s.', 'wp-fusion' ),
					esc_html( wp_fusion()->crm->name ),
					$contact_id
				);
			} else {
				...
				$message = sprintf(
					/* translators: %1$s CRM name */
					__( '<strong>Error:</strong> Unable to push user meta to %1$s: %2$s.', 'wp-fusion' ),
					esc_html( wp_fusion()->crm->name ),
					$result->get_error_message()
				);
	...
	public function user_new_form() {
		...
		<table class="form-table" role="presentation">
			...
			<tr>
				<th scope="row"><?php printf( esc_html__( 'Add to %s', 'wp-fusion' ), wp_fusion()->crm->name ); ?></th>
				<td>
					<input type="checkbox" name="wpf_add_contact" id="wpf_add_contact" value="1" <?php checked( wpf_get_option( 'create_users' ) ); ?> />
					<label for="wpf_add_contact"><?php printf( esc_html__( 'Add the user as a contact in %s', 'wp-fusion' ), wp_fusion()->crm->name ); ?></label>
				</td>
			</tr>
	...
	public function user_profile_update( $user_id ) {
		...
		if ( ! empty( $_POST['wpf_tags_field_edited'] ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$message = sprintf(
					/* translators: %1$s CRM name */
					__( 'Error: Unable to update tags in %1$s: %2$s.', 'wp-fusion' ),
					esc_html( wp_fusion()->crm->name ),
					$result->get_error_message()
				);
		...
		if ( isset( $_GET['newuseremail'] ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$message = sprintf(
					/* translators: %1$s CRM name */
					__( '<strong>Error:</strong> Unable to update user email in %1$s: %2$s.', 'wp-fusion' ),
					esc_html( wp_fusion()->crm->name ),
					$result->get_error_message()
				);
	...
	public function filter_form_fields( $post_data, $user_id ) {
		...
		if ( ! is_null( $screen ) && in_array( $screen->id, array( 'profile', 'user-edit', 'user-new', 'user' ) ) ) {
			...
			if ( 'user' === $screen->id && doing_filter( 'wpf_user_register' ) && ! isset( $_POST['wpf_add_contact'] ) ) {
				...
				wpf_log( 'notice', $user_id, sprintf( __( 'Add to %1$s was not checked, the user will not be synced to %1$s.', 'wp-fusion' ), wp_fusion()->crm->name ) );
	...
	public function user_profile( $user ) {
		...
		<h2 class="wp-fusion-user-profile-settings"><?php echo wpf_logo_svg(); ?> <?php esc_html_e( 'WP Fusion', 'wp-fusion' ); ?></h2>
		...
		<table class="form-table wp-fusion-user-profile-settings">
			...
			<tr>
				<th><label for="contact_id"><?php printf( esc_html__( '%s Contact ID', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ); ?></label></th>
				<td id="contact-id">
					...
					<?php if ( false !== $contact_id ) : ?>
						...
						<?php else : ?>
							...
							<?php if ( false !== $edit_url ) : ?>
								...
								- <a href="<?php echo esc_url( $edit_url ); ?>" target="_blank"><?php printf( esc_html__( 'View in %s', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ); ?> &rarr;</a>
					...
					<?php else : ?>
						...
						<?php printf( esc_html__( 'No %s contact record found.', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ); ?>
						...
						<a href="<?php echo esc_url( add_query_arg( '_wpnonce', wp_create_nonce( 'wpf_profile_action' ), admin_url( 'user-edit.php?user_id=' . $user->ID . '&wpf_profile_action=register' ) ) ); ?>">
							<?php esc_html_e( 'Create new contact', 'wp-fusion' ); ?>.
						</a>
			...
			<?php if ( wp_fusion()->user->get_contact_id( $user->ID ) ) : ?>
				...
				<tr id="wpf-tags-row">
					<th><label for="wpf_tags"><?php printf( esc_html__( '%s Tags', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ); ?></label></th>
					<td id="wpf-tags-td">
						...
						<p class="description"><?php esc_html_e( 'These tags are currently applied to the user in', 'wp-fusion' ); ?> <?php echo esc_html( wp_fusion()->crm->name ); ?> <a id="wpf-profile-edit-tags" href="#"><?php esc_html_e( 'Edit Tags', 'wp-fusion' ); ?></a></p>

					</td>
				</tr>
			<?php endif; ?>
			<tr>
				<th><label for="resync_contact"><?php esc_html_e( 'Resync Tags', 'wp-fusion' ); ?></label></th>
				<td>

					<a id="resync-contact" href="#" class="button button-default" data-user_id="<?php echo $user->ID; ?>"><?php esc_html_e( 'Resync Tags', 'wp-fusion' ); ?></a>
					<p class="description"><?php printf( __( 'If the contact ID or tags aren\'t in sync, click here to reset the local data and look up the contact again by email address in %s.', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ); ?></p>

				</td>
			</tr>

			<tr>
				<th><label for="resync_contact"><?php esc_html_e( 'Additional Actions', 'wp-fusion' ); ?></label></th>
				<td>
					...
					<a href="<?php echo esc_url( add_query_arg( '_wpnonce', wp_create_nonce( 'wpf_profile_action' ), admin_url( 'user-edit.php?user_id=' . $user->ID . '&wpf_profile_action=push' ) ) ); ?>">
						<?php esc_html_e( 'Push User Meta', 'wp-fusion' ); ?>
					</a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php printf( esc_attr__( 'Extracts any enabled meta fields from the database and syncs them to %s.', 'wp-fusion' ), esc_attr( wp_fusion()->crm->name ) ); ?>"></span> | 
					...
					<a href="<?php echo esc_url( add_query_arg( '_wpnonce', wp_create_nonce( 'wpf_profile_action' ), admin_url( 'user-edit.php?user_id=' . $user->ID . '&wpf_profile_action=pull' ) ) ); ?>">
						<?php esc_html_e( 'Pull User Meta', 'wp-fusion' ); ?>
					</a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php printf( esc_attr__( 'Loads any enabled meta fields from %s and saves them to the user record.', 'wp-fusion' ), esc_attr( wp_fusion()->crm->name ) ); ?>"></span> | 
					...
					<a href="<?php echo esc_url( add_query_arg( '_wpnonce', wp_create_nonce( 'wpf_profile_action' ), admin_url( 'user-edit.php?user_id=' . $user->ID . '&wpf_profile_action=show_meta' ) ) ); ?>">
						<?php esc_html_e( 'Show User Meta', 'wp-fusion' ); ?>
					</a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php esc_html_e( 'Displays all metadata found in the database for this user.', 'wp-fusion' ); ?>"></span> |

					<a href="<?php echo esc_url( admin_url( 'tools.php?page=wpf-settings-logs&user=' . $user->ID ) ); ?>">
						<?php esc_html_e( 'View Logs', 'wp-fusion' ); ?> &rarr;
					</a>
