...
class WPF_API {
	...
	public function handle_webhooks() {
		...
		if ( isset( $_REQUEST['wpf_action'] ) && 'thrivecart' != $_REQUEST['wpf_action'] ) {
			...
			$post_data = apply_filters( 'wpf_crm_post_data', $request );
			...
			if ( empty( $post_data ) || empty( $post_data['contact_id'] ) ) {
				...
				wpf_log(
					'error',
					0,
					'<strong>' . $action . '</strong> webhook received but contact data was not found or in an invalid format.',
					array(
						'source'              => 'api',
						'meta_array_nofilter' => $request,
					)
				);
	...
	public function thrivecart() {
		...
		if ( ! isset( $customer['email'] ) ) {
			...
			wpf_log(
				'error',
				0,
				'ThriveCart success URL detected but customer data was not found or in an invalid format.',
				array(
					'source'              => 'api',
					'meta_array_nofilter' => $request,
				)
