	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( empty( $options[ "{$this->slug}_refresh_token" ] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings[ "{$this->slug}_api_domain" ] = array(
				'title'   => __( 'API Domain URL', 'wp-fusion' ),
				'type'    => 'text',
				'section' => 'setup',
			);
			...
			if ( ! empty( $options['connection_configured'] ) && 'pipedrive' === wpf_get_option( 'crm' ) ) {
				...
				$new_settings['pipedrive_tag'] = array(
					'title'   => __( 'Tags Field', 'wp-fusion' ),
					'type'    => 'crm_field',
					'section' => 'setup',
					'desc'    => __( 'Select a tag field to be used for segmentation with WP Fusion. For more information, see <a href="https://wpfusion.com/documentation/" target="_blank">Tags with Pipedrive</a>.', 'wp-fusion' ),
				);
