	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( ! empty( $options['connection_configured'] ) ) {
			...
			$is_config['backup_api_key'] = array(
				'title'   => __( 'Backup Service Account Key', 'wp-fusion' ),
				'desc'    => __( 'Optional: Enter a backup Service Account Key to use if the primary key reaches its API limit. The system will automatically switch to this key if throttling is detected.', 'wp-fusion' ),
				'type'    => 'text',
				'section' => 'setup',
			);
		}
		...
		$is_config['api_key'] = array(
			'title'       => __( 'Service Account Key', 'wp-fusion' ),
			'desc'        => __( 'For help finding your Service Account key, please read <a target="_blank" href="https://developer.infusionsoft.com/pat-and-sak/">this knowledgebase article</a>.', 'wp-fusion' ),
			'type'        => 'api_validate',
			'section'     => 'setup',
			'class'       => 'api_key',
