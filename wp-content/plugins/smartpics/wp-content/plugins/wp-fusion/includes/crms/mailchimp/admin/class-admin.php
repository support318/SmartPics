...
class WPF_MailChimp_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( $settings['connection_configured'] && wpf_get_option( 'crm' ) === $this->slug ) {
			...
			$new_settings['mc_default_list'] = array(
				'title'       => __( 'MailChimp Audience', 'wp-fusion' ),
				'desc'        => __( 'Select an audience to use for WP Fusion. If you change the audience, you\'ll need to click Refresh Available Tags & Fields (below) to update the available dropdown options.', 'wp-fusion' ),
				'type'        => 'select',
	...
	public function register_settings( $settings, $options ) {
		...
		$new_settings = array(
			'mc_optin' => array(
				'title'   => __( 'Double Optin', 'wp-fusion' ),
				'desc'    => __( 'Send an optin confirmation email from MailChimp when new subscribers are added to your audience.', 'wp-fusion' ),
				'type'    => 'checkbox',
				'std'     => false,
				'section' => 'main',
			),
