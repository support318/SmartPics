...
class WPF_MailerLite_Admin {
	...
	public function register_connection_settings( $settings ) {
		...
		$new_settings = array(
			...
			'mailerlite_key'    => array(
				...
				'desc'        => __( 'You can find your API token in the <a href="https://dashboard.mailerlite.com/integrations/api" target="_blank">Developer API</a> settings of your MailerLite account.', 'wp-fusion' ),
				'type'        => 'api_validate',
				'section'     => 'setup',
	...
	public function register_settings( $settings, $options ) {
		...
		$new_settings['email_changes']['desc'] .= sprintf( __( 'For more information, %1$ssee our documentation%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/crm-specific-docs/email-address-changes-with-mailerlite/" target="_blank">', '</a>' );
		...
		if ( wp_fusion()->is_full_version() ) {
			...
			$settings['access_key_desc']['desc'] .= ' ' . sprintf( __( 'To list all registered webhooks (for debugging purposes), %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings&ml_debug=true' ) ) . '">', '</a>' );

			if ( isset( $_GET['ml_debug'] ) ) {

				$settings['access_key_desc']['desc'] .= ' ' . sprintf( __( 'To <strong>delete</strong> all registered webhooks, %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings&ml_debug=true&ml_destroy_all_webhooks=true' ) ) . '">', '</a>' );
			...
			$new_settings['mailerlite_add_tag'] = array(
				...
				'desc'        => __( 'When a contact is added to this group in MailerLite, they will be imported as a new WordPress user.', 'wp-fusion' ),
				'type'        => 'assign_tags',
				'section'     => 'main',
				'placeholder' => __( 'Select a group', 'wp-fusion' ),
				'limit'       => 1,
			);
		...
		$new_settings = array(
			'mailerlite_optin' => array(
				'title'   => __( 'Default Optin Status', 'wp-fusion' ),
				'desc'    => __( 'Select the default optin status for new contacts.', 'wp-fusion' ),
				'tooltip' => __( '"Default" will respect the opt-in settings configured in MailerLite. Set "Active" to mark the subscriber confirmed, or "Unconfirmed" to trigger a double-opt-in email.', 'wp-fusion' ),
				'type'    => 'radio',
				'section' => 'main',
				'choices' => array(
					''             => __( 'Default', 'wp-fusion' ),
					'active'       => __( 'Active', 'wp-fusion' ),
					'unconfirmed'  => __( 'Unconfirmed', 'wp-fusion' ),
					'unsubscribed' => __( 'Unsubscribed', 'wp-fusion' ),
				),
			),
		);

		$new_settings['mailerlite_optin']['desc'] .= ' ' . sprintf( __( 'For more information, %1$ssee our documentation%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/crm-specific-docs/mailerlite-double-opt-ins/" target="_blank">', '</a>' );

		$new_settings['mailerlite_resubscribe'] = array(
			'title'   => __( 'Resubscribe', 'wp-fusion' ),
			'desc'    => __( 'When adding a subscriber to a new group, resubscribe them in case they have unsubscribed.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'main',
		);
		...
		$new_settings['site_tracking_header'] = array(
			'title'   => __( 'MailerLite Site Tracking', 'wp-fusion' ),
			'type'    => 'heading',
			'section' => 'main',
		);
		...
		$new_settings['site_tracking'] = array(
			...
			'desc'    => __( 'Enable <a target="_blank" href="https://wpfusion.com/documentation/tutorials/site-tracking-scripts/#mailerlite">MailerLite site tracking</a>.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'main',
		);
