...
class WPF_KlickTipp {
		...
		if ( strpos( $url, 'api.klicktipp.com' ) !== false ) {
			...
			if ( ( 200 !== $code && 404 !== $code ) || ( 404 === $code && strpos( $url, '/search' ) !== false ) ) {
				...
				if ( '["API Zugriff verweigert."]' === wp_remote_retrieve_body( $response ) ) {
					return new WP_Error( 'error', __( 'Access denied. This user account does not have access to the KlickTipp API. See the installation guide (click View Documentation above) for more information.', 'wp-fusion' ) );
				}
	...
	public function sync_tags() {
		...
		if ( is_wp_error( $this->connect() ) ) {
			return new WP_Error( 'error', __( 'Unable to connect to KlickTipp.', 'wp-fusion' ) );
		}
	...
	public function apply_tags( $tags, $contact_id ) {
		...
		if ( false === $email ) {
			return new WP_Error( 'error', sprintf( __( 'Unable to find email address for contact ID %d. Can\'t apply tags.', 'wp-fusion' ), $contact_id ) );
		}
	...
	public function remove_tags( $tags, $contact_id ) {
		...
		if ( false === $email ) {
			return new WP_Error( 'error', sprintf( __( 'Unable to find email address for contact ID %d. Can\'t remove tags.', 'wp-fusion' ), $contact_id ) );
		}
	...
	public function add_contact( $data ) {
		...
		return new WP_Error( 'error', __( 'Unable to add contact.', 'wp-fusion' ) );
	}
	...
	public function load_contact( $contact_id ) {
		...
		if ( empty( $body ) ) {
			...
			return new WP_Error( 'error', sprintf( __( 'Unable to find contact ID %d.', 'wp-fusion' ), $contact_id ) );
		}
