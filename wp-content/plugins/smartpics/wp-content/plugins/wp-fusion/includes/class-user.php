...
class WPF_User {
	...
	public function user_register( $user_id, $post_data = array(), $force = false ) {
		...
		if ( ! wpf_get_option( 'create_users' ) && false === $force && empty( $contact_id ) ) {
			...
			wpf_log(
				...
				$user_id,
				/* translators: %s: CRM Name */
				sprintf( __( 'User registration not synced to %s because "Create Contacts" is disabled in the WP Fusion settings. You will not be able to apply tags to this user.', 'wp-fusion' ), wp_fusion()->crm->name )
			);
		...
		if ( empty( $contact_id ) ) {
			...
			if ( ! empty( $valid_roles ) && ! in_array( $post_data['role'], $valid_roles ) && false === $force ) {
				...
				wpf_log(
					...
					sprintf( __( 'User not added to %1$s because role %2$s isn\'t enabled for contact creation.', 'wp-fusion' ), wp_fusion()->crm->name, '<strong>' . $post_data['role'] . '</strong>' ),
					array(
			...
			if ( is_wp_error( $contact_id ) ) {
				...
				wpf_log(
					...
					sprintf( __( 'Error adding contact: %s', 'wp-fusion' ), $contact_id->get_error_message() ),
					array(
						'source' => 'user-register',
					)
				);
			...
			wpf_log(
				...
				sprintf( __( 'New user registration. Updating contact #%1$s in %2$s:', 'wp-fusion' ), $contact_id, wp_fusion()->crm->name ),
				array(
					'source'     => 'user-register',
					'meta_array' => $post_data,
				)
			...
			if ( is_wp_error( $result ) ) {
				...
				wpf_log(
					...
					$user_id,
					/* translators: %s: Error message */
					sprintf( __( 'Error updating contact: %s', 'wp-fusion' ), $result->get_error_message() ),
					array(
						'source' => 'user-register',
					)
				);
	...
	public function pull_user_meta( $user_id = false ) {
		...
		if ( empty( $contact_id ) ) {
			wpf_log( 'notice', $user_id, __( 'Error loading user meta: no contact record found.', 'wp-fusion' ) );
	...
	public function apply_tags( $tags, $user_id = false ) {
		...
		if ( apply_filters( 'wpf_prevent_reapply_tags', wpf_get_option( 'prevent_reapply', true ) ) ) {
			...
			if ( empty( $diff ) ) {
				...
				wpf_log( 'info', $user_id, __( 'Applying Tag(s). No API call will be sent since the user already has all of the following tag(s)', 'wp-fusion' ) . ': ', array( 'tag_array' => $tags ) );
				return true;
		...
		if ( doing_action( 'wpf_tags_modified' ) && ! doing_wpf_webhook() ) {
			wpf_log( 'notice', $user_id, __( '<strong>Chaining situation detected</strong>. WP Fusion is about to apply tags as the result of an automated enrollment, which was triggered by other tags being applied. This kind of setup should be avoided and may result in unexpected behavior or site instability.', 'wp-fusion' ) );
		}
	...
	public function remove_tags( $tags, $user_id = false ) {
		...
		if ( empty( $diff ) ) {
			...
			// wpf_log( 'info', $user_id, __( 'Removing Tag(s). No API call will be sent since the user already doesn\'t have the tag(s)', 'wp-fusion' ) . ': ', array( 'tag_array' => $tags ) );
			return true;
		}
		...
		if ( doing_action( 'wpf_tags_modified' ) && ! doing_wpf_webhook() ) {
			wpf_log( 'notice', $user_id, __( '<strong>Chaining situation detected</strong>. WP Fusion is about to remove tags as the result of an automated enrollment, which was triggered by other tags being applied. This kind of setup should be avoided and may result in unexpected behavior or site instability.', 'wp-fusion' ) );
		}
